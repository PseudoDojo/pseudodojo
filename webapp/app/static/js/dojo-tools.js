...
const ALL_KEYS = ['high', 'low', 'normal']; //, 'nv'];  // 'd', 'dp', 'gb'];
...
function dojo_start() {
    ...
    var a = $.getJSON("/api/repo-data");
    var b = $.getJSON("static/json/targz.json");
...
function load_set_info() {
    ...
    for (const elm of ALL_ELEMENTS) {
        if (elm == "Li") console.log(FILES[type][xcf][table][elm]);
        try {
            meta[elm] = FILES[type][xcf][table][elm]["hints"];
        }
        catch (error) {
            if (DEBUG) console.log("Cannot find element hints for:", elm, "with type:", type, ", xcf:", xcf, ", table:", table, "\n", error);
            meta[elm] = {};
        }
    }
...
function _get_pseudo_selection(dom_object){
  ...
  var table = $("#TABLE").val();
  var fmt = $("#FMT").val();
  let relativistic_type;

  // console.log(FILES[type][xcf][table])
  // console.log(FILES[type][xcf][table][fmt])

  try {
    var fpath = FILES[type][xcf][table][elm]["basename"];
    
    if (type.includes("SR")) {
      relativistic_type = ""
    }
    else {
      relativistic_type = "-FR"
    }

    var base_url = "https://raw.githubusercontent.com/PseudoDojo/ONCVPSP-"
    var url = base_url + xcf + relativistic_type + "-PDv0.4/master/" + elm + "/" + fpath; 
  } 
  catch (error) {
    var url = null;
    if (DEBUG) {
...
function build_ui(){
  ...
  $(document).ready(function($) {
    ...
    $('.plugin').on('click', function() {
      ...
      if (sel.fmt === 'out') {
          // For HTML, pass the path to the server to download the file
          // window.location.href = sel.url;
          // or window.open(sel.url, '_blank');
          var pseudo_fname = sel.url.replace(/\.psp8$/, ".out")

          // Send a POST request to Flask server with the file path
          fetch('/process_outfile', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
            },
            body: 'fname=' + encodeURIComponent(pseudo_fname)
          })
          .then(response => response.text())
          .then(htmlContent => {
            // Open the processed HTML content in a new tab/window
            var newWindow = window.open();
            newWindow.document.write(htmlContent);
          })
          .catch(error => {
            console.error('Error processing .out file:', error);
          ...
          // window.location.href = sel.url;
          // or window.open(sel.url, '_blank');
          var pseudo_fname = sel.url.replace(/\.psp8$/, ".in")
        ...
        // For HTML, pass the path to the server to download the file
          ...
          fetch(sel.url)
              ...
              .then(blob => {
                  ...
                  link.href = blobUrl;
                  link.download = extractFileName(sel.url); // Extract the file name from the URL
    ...
    function extractFileName(url) {
      return url.split('/').pop(); // Extracts the last segment of the URL as the file name
    }
